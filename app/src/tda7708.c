...
static uint8_t g_tda7708_i2c_error = 0;

static u8 g_tda7708_in_seek = 0;

static void tda7708_i2c_wait_ack()
{
	if (i2c_wait_ack()) {
		++g_tda7708_i2c_error;
	} else {
		g_tda7708_i2c_error=0;
	}
}
...
static void tda7708_i2c_read(u8 len)
{
	...
	if (g_tda7708_i2c_error>16) {
		return;
	}
	
	i2c_start();
	i2c_send_byte(TDA7708_I2C_ADDR_W);
	tda7708_i2c_wait_ack();
...
//					} else {
						for (x=PRESET_BAND_FM1; x<=PRESET_BAND_FM3; x++) {
							for(y=0;y<FM_PRESET_NUM;y++) {
								g_radio_info.preset_fm[x-PRESET_BAND_FM1][y]=3;
							}
						}
//					}
//				}
