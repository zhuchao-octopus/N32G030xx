...
#endif
#ifdef SUPPORT_IR_RX_6B86
static const IR_RX_KEY_MAP g_ir_rx_key_map_6b86[] = {
	{0x10, UICC_FAKE_POWER_OFF},
	{0x12, UICC_SOURCE},
	{0x1B, UICC_VOLUME_UP},
	{0x1A, UICC_VOLUME_DOWN},
	{0x05, UICC_MUTE},
	{0x04, UICC_PREV},
	{0x06, UICC_NEXT},
	{0x08, UICC_RADIO},
	{0x0A, UICC_BT},
	{0x14, UICC_RADIO_SCAN},
	{0x16, UICC_EQ},
};
#endif
...
#ifdef SUPPORT_IR_RX_6B86
					if ( 
						(g_ir_rx_info.sys_code==IR_SYS_CODE_4)	/* system code 4 */
						&&( (0xE41B == g_ir_rx_info.key_code) ||	/* Volume + */
							(0xE51A == g_ir_rx_info.key_code) ) 	/* Volume - */
					  )
					{
						repeat = TRUE;
					}
...
#endif
#ifdef SUPPORT_IR_RX_6B86
		} else if (g_ir_rx_info.sys_code==IR_SYS_CODE_4) {
			len = sizeof(g_ir_rx_key_map_6b86)/sizeof(IR_RX_KEY_MAP);
			for(i=0;i<len;i++) {
				if(code==g_ir_rx_key_map_6b86[i].ir_key_code) {
					PostEvent(MMI_MODULE,g_ir_rx_key_map_6b86[i].uicc_key_code,0);		
					break;
				}
			}
#endif
