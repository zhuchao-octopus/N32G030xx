...
static void audio_volume_handler(void)
{
	...
	if (g_soc_temp > 105) {
		max_vol = 20;
	} else if (g_soc_temp > 103) {
		max_vol = 22;
	} else if (g_soc_temp > 102) {
		max_vol = 24;
	} else if (g_soc_temp > 100) {
		max_vol = 25;
	} else if (g_soc_temp > 98) {
		max_vol = 26;
	} else if (g_soc_temp > 96) {
		max_vol = 27;
	} else if (g_soc_temp > 94) {
		max_vol = 28;
	} else if (g_soc_temp > 92) {
		max_vol = 29;
	} else {
		max_vol = 30;
	}
...
void audio_main(void)
{
	...
	if (AUDIO_STATE_INIT_ING == g_audio_info.state) {
		if (g_audio_info.pwr_timer < T1S_12) {
