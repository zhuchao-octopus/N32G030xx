...
static uint16_t correct_fm_freq(uint16_t freq)
{
	if (IS_RUSSIA_RADIO()) {
		freq = _correct_fm_freq_for_russia(freq);
	} else {
		if ((freq) == 0)  (freq) = FM_MIN_FREQ;
		if ((freq-FM_MIN_FREQ)%FM_STEP) freq=freq-((freq-FM_MIN_FREQ)%FM_STEP);
		if ((freq) > FM_MAX_FREQ)  (freq) = FM_MIN_FREQ;
		if ((freq) < FM_MIN_FREQ)  (freq) = FM_MAX_FREQ;
	}
	return freq;
}
static uint16_t correct_am_freq(uint16_t freq)
{
	if ((freq) == 0)  (freq) = AM_MIN_FREQ;
	if ((freq-AM_MIN_FREQ)%AM_STEP) freq=freq-((freq-AM_MIN_FREQ)%AM_STEP);
	if ((freq) > AM_MAX_FREQ)  (freq) = AM_MIN_FREQ;
	if ((freq) < AM_MIN_FREQ)  (freq) = AM_MAX_FREQ;
	return freq;
}
static s8 correct_fm_preset_num(s8 n)
{
	if ((n) >= FM_PRESET_NUM)  (n) = 0;
	if ((n) < 0) (n) = (FM_PRESET_NUM-1);
	return n;
}
static s8 correct_am_preset_num(s8 n)
{
...
static void reset_seek_flags(void)
{
...
static void radio_config_fix_pop(u8 timer)
{
	...
	audio_set_mute_temporary(timer*24);
...
static void radio_handle_freq_set(void)
{
	...
	switch(G_RFSET_STATE()) {
		...
		case RFSET_STATE_BEGIN:
			...
			if (IS_IN_FM(VAR_RFSET_BAND)) {
				VAR_RFSET_FREQ=correct_fm_freq(VAR_RFSET_FREQ);
				radio_dev_set_freq_tune(RADIO_BAND_FM, VAR_RFSET_FREQ);
			} else {
				VAR_RFSET_FREQ = correct_am_freq(VAR_RFSET_FREQ);
...
static void radio_handle_freq_seek(void)
{
	...
	switch (G_RFS_STATE()) {
		...
		case RFS_STATE_WAIT_RDS_PTY_MATCH:
			if ((0 == g_rds_info.pty_select) || 
				...
				switch(G_RFS_CMD()) {
					case RFS_CMD_AUTO_SAVE:
						if (IS_IN_FM(VAR_RFS_BAND)) {
							G_CUR_IDX(VAR_RFS_BAND) = correct_fm_preset_num(G_CUR_IDX(VAR_RFS_BAND));
							S_PRESET_FM(VAR_RFS_BAND, G_CUR_IDX(VAR_RFS_BAND), VAR_RFS_C_FREQ);
						} else if (IS_IN_AM(VAR_RFS_BAND)) {
							G_CUR_IDX(VAR_RFS_BAND) = correct_am_preset_num(G_CUR_IDX(VAR_RFS_BAND));
		...
		case RFS_STATE_GOTO_NEXT_FREQ:
			...
			if (IS_IN_FM(VAR_RFS_BAND)) {
				VAR_RFS_C_FREQ=correct_fm_freq(VAR_RFS_C_FREQ);
			} else if (IS_IN_AM(VAR_RFS_BAND)) {
				VAR_RFS_C_FREQ = correct_am_freq(VAR_RFS_C_FREQ);
			}
	...
	switch(pEvt->ID) {
		...
		case RADIO_EVT_PRESET_SELECT:
			...
			if (IS_IN_FM(G_CUR_BAND())) {
				tmp_id = correct_fm_preset_num(tmp_id);
				tmp_freq = G_PRESET_FM(G_CUR_BAND(), tmp_id);
			} else if (IS_IN_AM(G_CUR_BAND())) {
				tmp_id = correct_am_preset_num(tmp_id);
		...
		case RADIO_EVT_FREQ_SET:
			if (IS_IN_FM(G_CUR_BAND())) {
				tmp_freq = correct_fm_freq(pEvt->prm);
			} else if (IS_IN_AM(G_CUR_BAND())) {
				tmp_freq = correct_am_freq(pEvt->prm);
			}			
			if (G_CUR_FREQ() != tmp_freq) {
				g_radio_info.state = RADIO_STATE_FREQ_SETTING;
#ifdef FIX_RADIO_BAND_SWITCH_POP
...
void radio_init_area(u8 area)
{
	...
	S_CUR_FREQ_BY_BAND(PRESET_BAND_FM1, FM_MIN_FREQ);
	S_CUR_FREQ_BY_BAND(PRESET_BAND_FM2, FM_MIN_FREQ);
	S_CUR_FREQ_BY_BAND(PRESET_BAND_FM3, FM_MIN_FREQ);
	S_CUR_FREQ_BY_BAND(PRESET_BAND_AM1, AM_MIN_FREQ);
	S_CUR_FREQ_BY_BAND(PRESET_BAND_AM2, AM_MIN_FREQ);

	for (i=PRESET_BAND_FM1; i<=PRESET_BAND_FM3; i++) {
		for(j=0;j<FM_PRESET_NUM;j++) {
			S_PRESET_FM(i, j, FM_MIN_FREQ);
		}
	}
	for (i=PRESET_BAND_AM1; i<=PRESET_BAND_AM2; i++) {
		for(j=0;j<AM_PRESET_NUM;j++) {
			S_PRESET_AM(i, j, AM_MIN_FREQ);
		}
	}
	...
	switch (area) {
		case RADIO_AREA_USA:
			for (i=PRESET_BAND_FM1; i<=PRESET_BAND_FM3; i++) {
				S_PRESET_FM(i, 0, 8750);
				S_PRESET_FM(i, 1, 9010);
				S_PRESET_FM(i, 2, 9810);
				S_PRESET_FM(i, 3, 10610);
				S_PRESET_FM(i, 4, 10790);
			}
			break;
		case RADIO_AREA_LATIN:
		case RADIO_AREA_EUROPE:
		...
		case RADIO_AREA_ASIA_PACIFIC:
			for (i=PRESET_BAND_FM1; i<=PRESET_BAND_FM3; i++) {
				...
				S_PRESET_FM(i, 2, 9810);
				S_PRESET_FM(i, 3, 10610);
				S_PRESET_FM(i, 4, 10800);
			}
			break;
		case RADIO_AREA_OIRT:
			for (i=PRESET_BAND_FM1; i<=PRESET_BAND_FM3; i++) {
				S_PRESET_FM(i, 0, 6500);
				S_PRESET_FM(i, 1, 6710);
				S_PRESET_FM(i, 2, 7040);
				S_PRESET_FM(i, 3, 7250);
				S_PRESET_FM(i, 4, 7400);
			}
			break;
		case RADIO_AREA_JAPAN:
		case RADIO_AREA_JAPAN_2:
			for (i=PRESET_BAND_FM1; i<=PRESET_BAND_FM3; i++) {
				S_PRESET_FM(i, 0, 7600);
				S_PRESET_FM(i, 1, 7640);
				S_PRESET_FM(i, 2, 8560);
				S_PRESET_FM(i, 3, 8700);
				S_PRESET_FM(i, 4, 9000);
			}
			break;
		case RADIO_AREA_KOREA:
			for (i=PRESET_BAND_FM1; i<=PRESET_BAND_FM3; i++) {
				S_PRESET_FM(i, 0, 8810);
				S_PRESET_FM(i, 1, 9010);
				S_PRESET_FM(i, 2, 9810);
				S_PRESET_FM(i, 3, 10610);
				S_PRESET_FM(i, 4, 10790);
		...
		case RADIO_AREA_RUSSIA:
			for (i=PRESET_BAND_FM1; i<=PRESET_BAND_FM3; i++) {
				...
				S_PRESET_FM(i, 3, 7250);
				S_PRESET_FM(i, 4, 7400);
				S_PRESET_FM(i, 5, 6500);
				S_PRESET_FM(i, 6, 8750);
				S_PRESET_FM(i, 7, 9010);
				S_PRESET_FM(i, 8, 9810);
				S_PRESET_FM(i, 9, 10610);
				S_PRESET_FM(i, 10, 10800);
			}
			break;
	}
	...
	switch (area) {
		...
		case RADIO_AREA_LATIN:
			for (i=PRESET_BAND_AM1; i<=PRESET_BAND_AM2; i++) {
				S_PRESET_AM(i, 0, 530);
				S_PRESET_AM(i, 1, 600);
				S_PRESET_AM(i, 2, 1000);
				S_PRESET_AM(i, 3, 1400);
				S_PRESET_AM(i, 4, 1710);
			}
			break;
		...
		case RADIO_AREA_KOREA:
			for (i=PRESET_BAND_AM1; i<=PRESET_BAND_AM2; i++) {
				...
				S_PRESET_AM(i, 1, 603);
				S_PRESET_AM(i, 2, 999);
				S_PRESET_AM(i, 3, 1404);
				S_PRESET_AM(i, 4, 1611);
			}
			break;
		case RADIO_AREA_SOUTH_AMERICA:
		case RADIO_AREA_ASIA_PACIFIC:
...
void radio_init(void)
{
	...
	if (g_radio_area>=RADIO_AREA_NUMS) {
		g_radio_area = RADIO_AREA_EUROPE;
	}
	radio_init_area(g_radio_area);
...
void radio_main(void)
{
	...
	if (RADIO_STATE_PWR_ON_ING == g_radio_info.state) {
		...
		if (1==g_sys_info_store.radio_ant_ctrl) {
			GPIO_SetBits(GPIO_RADIO_ANT_CTRL_GRP, GPIO_RADIO_ANT_CTRL_PIN);
		} else {
	...
	if (RADIO_STATE_INIT_ING == g_radio_info.state) {
		...
		if (g_radio_info.pwr_timer < T1S5_12) {
