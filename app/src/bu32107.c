...
static const s8 g_bu32107_gains[] = {
	-79,	/* volume 0 */
	-65, -52, -45, -39, -34,	/* volume 1~5 */
	-30, -28, -26, -24, -22,	/* volume 6~10 */
	-21, -20, -19, -18, -17,	/* volume 11~15 */
	-16, -15, -14, -13, -12,		/* volume 16~20 */
	-11, -10, -9, -8, -7,		/* volume 21~25 */
	-6, -4, -2, 0, 2		/* volume 26~30 */
};
static const u8 g_bu32107_atten_table[8] = {0, 3, 6, 9, 12, 16, 20, 63};

static uint8_t g_bu32107_i2c_error = 0;
static void bu32107_i2c_wait_ack()
{
	if (i2c_wait_ack()) {
		++g_bu32107_i2c_error;
	} else {
		g_bu32107_i2c_error=0;
	}
}

static void bu32107_write(u16 addr, u8 value)
{
	if (g_bu32107_i2c_error>16) {
		return;
	}

	i2c_start();
	i2c_send_byte(BU32107_I2C_W_ADDR);
	bu32107_i2c_wait_ack();
...
static void bu32107_update_time_alignment(void)
{
	...
	for (cnt=0; cnt<AUDIO_SPK_NUMS; cnt++) {
		...
		bu32107_write(BU32107_TA_FL_H+2*cnt, MSB(val));
		bu32107_write(BU32107_TA_FL_L+2*cnt, LSB(val));
	}
	...
	bu32107_write(BU32107_TA_RL2_H, MSB(val));
	bu32107_write(BU32107_TA_RL2_L, LSB(val));
	bu32107_write(BU32107_TA_RR2_H, MSB(val));
	bu32107_write(BU32107_TA_RR2_L, LSB(val));
...
bool audio_dev_init(void)
{
	...
	bu32107_write(BU32107_INIT_SETUP_2, 0x08);	// Loudness bypass
...
//	bu32107_write(BU32107_INPUT_GAIN, 0x00);
	bu32107_write(0x0102, 0x20);	// SL/SL -> SL/SR
	bu32107_write(BU32107_ANA_INPUT_SEL, 0x00);
...
//	bu32107_write(0x0108, 0x80);
	bu32107_write(BU32107_DIG_MIX_SEL, 0xC0);	// DAC digital mix mode:ON, FL/FR mix OFF
//	bu32107_write(BU32107_AVOL_DMIX, 0x80);
//	bu32107_write(0x0200, 0x00);	// disable digital ext output&input
//	bu32107_write(0x0201, 0x00);	// disable digital output 1&3
	bu32107_write(BU32107_DIG_IO_SEL3, 0x66);	// digital input 3: C-1; digital input 1: C-1
	bu32107_write(BU32107_DSP_SEL_1, 0x08);	// time-alignment 4-ch mode
//	bu32107_write(0x0204, 0x00);
	bu32107_write(BU32107_DSP_SEL_3, 0x05);	// time-alignment -> p2bass
	bu32107_write(0x0206, 0x32);	// Digital Mixing -> SR; Digital Stereo mix (L+R)
//	bu32107_write(0x0207, 0x00);
//	bu32107_write(0x0208, 0x00);
//	bu32107_write(BU32107_TA_FL_H, 0x00);
//	bu32107_write(BU32107_TA_FL_L, 0x00);
//	bu32107_write(BU32107_TA_FR_H, 0x00);
//	bu32107_write(BU32107_TA_FR_L, 0x00);
//	bu32107_write(BU32107_TA_RL_H, 0x00);
//	bu32107_write(BU32107_TA_RL_L, 0x00);
//	bu32107_write(BU32107_TA_RR_H, 0x00);
//	bu32107_write(BU32107_TA_RR_L, 0x00);
//	bu32107_write(0x0408, 0x00);
//	bu32107_write(0x0409, 0x00);
//	bu32107_write(BU32107_TA_RL2_H, 0x00);
//	bu32107_write(BU32107_TA_RL2_L, 0x00);
//	bu32107_write(BU32107_TA_RR2_H, 0x00);
//	bu32107_write(BU32107_TA_RR2_L, 0x00);
//	bu32107_write(0x0500, 0x04);
//	bu32107_write(0x0501, 0x10);		// SA2: 36dB
//	bu32107_write(0x0600, 0x00);		// 13-Band EQ + TONE

#if 1	// reduce code space
	for (i=0; i<16; i++) {
		bu32107_write(BU32107_EQ_50HZ+i, 0x40);
	}
#else
	bu32107_write(BU32107_EQ_50HZ, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_80HZ, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_125HZ, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_200HZ, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_315HZ, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_500HZ, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_800HZ, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_1K25, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_2K, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_3K15, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_5K, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_8K, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_12K5, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_BASS_F, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_MID, 0x40);		// Front & Rear
	bu32107_write(BU32107_EQ_TREBLE_F, 0x40);		// Front & Rear
#endif
...
//	bu32107_write(BU32107_EQ_BASS_R, 0x00);
//	bu32107_write(0x062E, 0x00);
//	bu32107_write(BU32107_EQ_TREBLE_R, 0x00);
//	bu32107_write(0x0700, 0x00);
	bu32107_write(0x0701, 0x55);		// Loudness: LPF 100HZ, HPF 10KHZ
	bu32107_write(BU32107_LOUDNESS, 0xA0);		// Loudness: HiBoost=0.55, 0dB
//	bu32107_write(BU32107_FRONT_HPF, 0x00);		// Front HPF through
//	bu32107_write(BU32107_REAR_HPF, 0x00);		// Rear HPF through
//	bu32107_write(BU32107_FRONT_P2BASS, 0x80);		// Front P2BASS through
//	bu32107_write(BU32107_REAR_P2BASS, 0x80);		// Rear P2BASS through
//	bu32107_write(0x0707, 0x00);
//	bu32107_write(0x0708, 0x00);
//	bu32107_write(BU32107_IIR, 0x00);		// IIR A/IIR B through
...
#if 1	// reduce code space
	for (i=0; i<6; i++) {
		bu32107_write(BU32107_DVOL_ATT_FL+i, 0x40);
	}
	for (i=0; i<6; i++) {
		bu32107_write(BU32107_DVOL_BOOST_FL+i, 0x80);
	}
#else
	bu32107_write(BU32107_DVOL_ATT_FL, 0x40);
	bu32107_write(BU32107_DVOL_ATT_FR, 0x40);
	bu32107_write(BU32107_DVOL_ATT_RL, 0x40);
	bu32107_write(BU32107_DVOL_ATT_RR, 0x40);
	bu32107_write(0x0904, 0x40);
	bu32107_write(0x0905, 0x40);
	bu32107_write(BU32107_DVOL_BOOST_FL, 0x80);
	bu32107_write(BU32107_DVOL_BOOST_FR, 0x80);
	bu32107_write(BU32107_DVOL_BOOST_RL, 0x80);
	bu32107_write(BU32107_DVOL_BOOST_RR, 0x80);
	bu32107_write(BU32107_DVOL_BOOST_SL, 0x80);
	bu32107_write(BU32107_DVOL_BOOST_SR, 0x80);
//	bu32107_write(BU32107_FADER_VOL_FL, 0x80);		// mute FL
//	bu32107_write(BU32107_FADER_VOL_FR, 0x80);		// mute FR
//	bu32107_write(BU32107_FADER_VOL_RL, 0x80);		// mute RL
//	bu32107_write(BU32107_FADER_VOL_RR, 0x80);		// mute RR
//	bu32107_write(BU32107_FADER_VOL_SL, 0x80);		// mute SL
#endif
	...
	bu32107_write(BU32107_IIR, 0x80);		// IIR A/IIR B coef
	for (i=0; i<2; i++) {
...
void audio_dev_update_source(AUDIO_SOURCE src)
{
	...
	//select analog input
	//val = bu32107_read(BU32107_ANA_INPUT_SEL);
	//val &= ~(7<<0);
	val = 0;
	switch (src) {
	...
	bu32107_write(BU32107_ANA_INPUT_SEL, val);

	// select DSP input
	//val = bu32107_read(BU32107_DSP_SEL_1);
	val = 0x08;
	switch (src) {
		case AUDIO_SRC_RADIO:
		case AUDIO_SRC_AUXIN:
	...
	bu32107_write(BU32107_DSP_SEL_1, val);
	...
	bu32107_write(BU32107_INPUT_GAIN, val);

	// modify digital IO selector if needed
	if (g_fake_pwr_off&&(!g_audio_info.bt_phone_on)) {
		bu32107_write(BU32107_DIG_IO_SEL3, 0x00);
	} else {
		bu32107_write(BU32107_DIG_IO_SEL3, 0x66);
	}
...
#if 1	// reduce code space
	for (cnt=0; cnt<4; cnt++) {
		bu32107_write(BU32107_FADER_VOL_FL+cnt, val_fader);
	}
#else
	bu32107_write(BU32107_FADER_VOL_FL, val_fader);
	bu32107_write(BU32107_FADER_VOL_FR, val_fader);
	bu32107_write(BU32107_FADER_VOL_RL, val_fader);
	bu32107_write(BU32107_FADER_VOL_RR, val_fader);
#endif
	bu32107_write(BU32107_FADER_VOL_SL, val_fader_sub);
#if 1	// reduce code space
	for (cnt=0; cnt<4; cnt++) {
		bu32107_write(BU32107_DVOL_BOOST_FL+cnt, val_dvol_boost);
	}
#else
	bu32107_write(BU32107_DVOL_BOOST_FL, val_dvol_boost);
	bu32107_write(BU32107_DVOL_BOOST_FR, val_dvol_boost);
	bu32107_write(BU32107_DVOL_BOOST_RL, val_dvol_boost);
	bu32107_write(BU32107_DVOL_BOOST_RR, val_dvol_boost);
#endif
	bu32107_write(BU32107_DVOL_BOOST_SL, val_dvol_boost_sub);
...
static void bu32107_update_mix_vol(u8 vol)
{
	...
	// value_dig_mix_sel = bu32107_read(BU32107_DIG_MIX_SEL);
	value_dig_mix_sel = 0xC0;
	if (0==vol) {
		value_dig_mix_sel &= ~(3<<4);
		bu32107_write(BU32107_DIG_MIX_SEL, value_dig_mix_sel);
	} else {
		gain = g_bu32107_gains[vol];
		...
		if (gain>=0) {
			value = 0xA0;
			bu32107_write(BU32107_AVOL_DMIX, value);

			value = 0x80-gain*2;
			bu32107_write(BU32107_DVOL_BOOST_SR, value);
		} else {
...
static u8 bu32107_calc_eq_level_to_reg_gain(u8 val, u8 level)
{
	...
	if (level >= DEFAULT_EQ_LEVEL) {
		val += (level-DEFAULT_EQ_LEVEL)/3;
	} else {
		val += (DEFAULT_EQ_LEVEL-level)/3;
		val |= (1<<4);
	}
...
void audio_dev_update_fader_balance(u8 fad, u8 bal)
{
	...
	if ((!g_audio_info.bt_phone_on)&&(g_audio_info.navi_break_on)) {
	...
	for (cnt=0; cnt<AUDIO_SPK_NUMS; cnt++) {
		...
		bu32107_write(BU32107_DVOL_ATT_FL+cnt, val);
...
void audio_dev_update_loudness(u8 loud)
{
	...
	//val &= ~0x0F;
	val = 0xA0 | (BU32107_LOUDNESS_STEP*g_audio_info.loudness);
	bu32107_write(BU32107_LOUDNESS, val);

	val = 0x08;//val = g_regs[BU32107_INIT_SETUP_2].value;
	if (0==loud) {
		val |= (1<<3);
	} else {
		val &= ~(1<<3);
	}
	bu32107_write(BU32107_INIT_SETUP_2, val);
...
void audio_dev_update_dsp_settings_1(void)
{
	...
	bu32107_write(BU32107_FRONT_HPF, val);
	bu32107_write(BU32107_REAR_HPF, val);
	...
	bu32107_write(BU32107_FRONT_P2BASS, val);
	bu32107_write(BU32107_REAR_P2BASS, val);
	...
	bu32107_write(BU32107_EQ_50HZ, val);
	...
	bu32107_write(BU32107_EQ_MID, val);
...
void audio_dev_update_dsp_settings_2(void)
{
	...
	if (AUDIO_SR_MODE_FLAT == g_audio_info.surround_mode) {
		//	// time-alignment -> p2bass rear
		bu32107_write(BU32107_DSP_SEL_3, 0x05);
	} else {
		//	// surround -> p2bass rear
		bu32107_write(BU32107_DSP_SEL_3, 0x25);
	}
	...
	switch (g_audio_info.surround_mode) {
		case AUDIO_SR_MODE_FLAT:
			//	// front/rear -HZ 0dB
			bu32107_write(BU32107_EQ_BASS_F, 0x40);
			bu32107_write(BU32107_EQ_TREBLE_F, 0x40);
			break;
		case AUDIO_SR_MODE_RECITAL:
			//	// 63HZ 4dB
			bu32107_write(BU32107_EQ_BASS_F, 0x12);
			//	// 2.5K 4dB
			bu32107_write(BU32107_EQ_TREBLE_F, 0x02);
			//	// 63HZ 4dB
			bu32107_write(BU32107_EQ_BASS_R, 0x12);
			//	// -HZ 0dB
			bu32107_write(BU32107_EQ_TREBLE_R, 0x00);
			break;
		case AUDIO_SR_MODE_CONCERT:
			//	// 100HZ 4dB
			bu32107_write(BU32107_EQ_BASS_F, 0x22);
			//	// 6.3K 4dB
			bu32107_write(BU32107_EQ_TREBLE_F, 0x22);
			//	// 100HZ 4dB
			bu32107_write(BU32107_EQ_BASS_R, 0x22);
			//	// 6.3K 4dB
			bu32107_write(BU32107_EQ_TREBLE_R, 0x22);
			break;
		case AUDIO_SR_MODE_BGM:
			//	// 100HZ -4dB
			bu32107_write(BU32107_EQ_BASS_F, 0x2A);
			//	// -HZ 0dB
			bu32107_write(BU32107_EQ_TREBLE_F, 0x00);
			//	// -HZ 0dB
			bu32107_write(BU32107_EQ_BASS_R, 0x00);
			//	// 10K -4dB
			bu32107_write(BU32107_EQ_TREBLE_R, 0x3A);
			break;
		case AUDIO_SR_MODE_MOVIE:
			//	// 100HZ 4dB
			bu32107_write(BU32107_EQ_BASS_F, 0x22);
			//	// 2.5K 6dB
			bu32107_write(BU32107_EQ_TREBLE_F, 0x03);
			//	// 100HZ 4dB
			bu32107_write(BU32107_EQ_BASS_R, 0x22);
			//	// -HZ 0dB
			bu32107_write(BU32107_EQ_TREBLE_R, 0x00);
		...
		case AUDIO_SR_MODE_DRAMA:
			//	// -HZ 0dB
			bu32107_write(BU32107_EQ_BASS_F, 0x00);
			//	// 2.5K 6dB
			bu32107_write(BU32107_EQ_TREBLE_F, 0x03);
			//	// 40HZ 4dB
			bu32107_write(BU32107_EQ_BASS_R, 0x02);
			//	// -HZ 0dB
			bu32107_write(BU32107_EQ_TREBLE_R, 0x00);
			break;
	}
	...
	switch (g_audio_info.surround_mode) {
		...
		case AUDIO_SR_MODE_DRAMA:
			//	// IIR through
			bu32107_write(BU32107_IIR, 0x00);
			break;
		default:
			//	// IIR coef
			bu32107_write(BU32107_IIR, 0x80);
			break;
	}
#else
	switch (g_audio_info.surround_mode) {
