...
#define RADIO_ANT_PWR_INFO		0x07
...
#define SUBID_RADIO_ANT_PWR_CTRL 0x05
	...
	switch (Group_ID_temp)
			...
			switch (Sub_ID_temp)
				...
				case SUBID_BLUETOOTH_CMD:
					...
					//					}
						...
						g_audio_info.carplay_phone_on = TRUE;
						}
		...
		case MCU_RX_GROUPID_RADIO_EVENT:
			switch (Sub_ID_temp)
				...
				case SUBID_RADIO_ANT_PWR_CTRL:
					switch (*Read_Lin_Ptr)
						{
						case 0x00:
						case 0x01:
							if ((*Read_Lin_Ptr) != g_sys_info_store.radio_ant_ctrl)
								{
								g_sys_info_store.radio_ant_ctrl = (*Read_Lin_Ptr);
								ak_flash_save_info();
								}
							...
							PostEvent(WINCE_MODULE, TX_TO_GUI_RADIO_ANT_PWR_INFO, g_sys_info_store.radio_ant_ctrl);
							break;
			...
			switch (Sub_ID_temp)
				...
				case SUBID_NAVI_AUDIO_NOTIFY:
					switch (*Read_Lin_Ptr)
						...
						case 0:
						case 1:
							audio_set_navi_break(*Read_Lin_Ptr);
							break;
				...
				case SUBID_APP_AUDIO_NOTIFY:
					if (0x66 == *Read_Lin_Ptr)
						{
						g_audio_info.android_snd_on = (TRUE);
						}
						...
						{
						audio_set_app_3rd_break((*Read_Lin_Ptr++ != 0) ? 1: 0);
						}
				...
				case SUBID_DSP_SETTINGS:
					...
					switch (*Read_Lin_Ptr++)
						...
						case 0x08:
							audio_set_dsp_speaker(AUDIO_SPK_FL, *Read_Lin_Ptr, * (Read_Lin_Ptr + 1),
								 * (Read_Lin_Ptr + 2));
							PostEvent(WINCE_MODULE, TX_TO_GUI_AUDIO_DSP_SETTINGS_2, 1);
							break;

						case 0x09:
							audio_set_dsp_speaker(AUDIO_SPK_FR, *Read_Lin_Ptr, * (Read_Lin_Ptr + 1),
								 * (Read_Lin_Ptr + 2));
							PostEvent(WINCE_MODULE, TX_TO_GUI_AUDIO_DSP_SETTINGS_2, 1);
							break;

						case 0x0A:
							audio_set_dsp_speaker(AUDIO_SPK_RL, *Read_Lin_Ptr, * (Read_Lin_Ptr + 1),
								 * (Read_Lin_Ptr + 2));
							PostEvent(WINCE_MODULE, TX_TO_GUI_AUDIO_DSP_SETTINGS_2, 1);
							break;

						case 0x0B:
							audio_set_dsp_speaker(AUDIO_SPK_RR, *Read_Lin_Ptr, * (Read_Lin_Ptr + 1),
								 * (Read_Lin_Ptr + 2));
							PostEvent(WINCE_MODULE, TX_TO_GUI_AUDIO_DSP_SETTINGS_2, 1);
							break;
				...
				case SUBID_SA_CTRL:
					g_audio_info.sa_en = ((bool) (*Read_Lin_Ptr));
					break;
		...
		case MCU_RX_GROUP_CAN_EVENT:
			switch (Sub_ID_temp)
				...
				case SUBID_CAN_PROTOCOL_SET:
					canbox_set_protocol((CAN_HEAD_TYPE) (* (Read_Lin_Ptr + 0)), 
			...
			switch (Sub_ID_temp)
				...
				case SUBID_SETTING_INFO_REQ:
						...
						switch (*Read_Lin_Ptr)
							...
							case 0x09:
								PostEvent(WINCE_MODULE, TX_TO_GUI_SYSTEM_NAVI_MIX_INFO,
									 g_audio_info.navi_mix_extra_gain);
								break;
				...
				case SUBID_LED_ONOFF_CTRL:
					switch (*Read_Lin_Ptr)
						...
						case 0x01:
						case 0x02:
							if ((*Read_Lin_Ptr) != g_sys_info_store.led_always_on)
								{
								g_sys_info_store.led_always_on = *Read_Lin_Ptr;
								ak_flash_save_info();
								}
			...
			switch (Sub_ID_temp)
				...
				case SUBID_SECRET_SETTING_INFO_REQ:
					switch (*Read_Lin_Ptr)
						...
						case 0x04:
							PostEvent(WINCE_MODULE, TX_TO_GUI_SECRET_SETTING_BREAK_DET_SET_INFO,
								 g_sys_info_store.brake_mode);
							break;
...
static void __host_tx_wait_finished(void)
{
	uint16_t		timeout;
...
void LinUart_Ack_Tx(uchar ackchar, uchar Seqence_num)
{
	...
	u8				buf[6] =
		{
		LIN_ADDRESS_MCU, LIN_ADDRESS_WINCE, 6, 0, 0, 0
		};
	u8				cnt;
	...
	for (cnt = 0; cnt < 5; cnt++)
		{
		checksum			^= buf[cnt];
		USART_SendData(HOST_COMM_UART, buf[cnt]);
		__host_tx_wait_finished();
		}
...
const uchar 	Mcu_Tx_Comd_Matrix[][4] =
{
	...
	TX_TO_GUI_RADIO_ANT_PWR_INFO, MCU_TX_RADIO_INFO, RADIO_ANT_PWR_INFO, 1, 
...
ext void LinTxWince_Service(void)
{
	...
	switch (nEvt->ID)
		...
		case TX_TO_GUI_RADIO_ANT_PWR_INFO:
			buff[0] = nEvt->prm;
			break;
